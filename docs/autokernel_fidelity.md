...
| Correctness and timing from one execution (`bench.py`) | `run_gate` returns (gates, steps/s) from one run; `benchmark` publishes the timing only after a green verify |
