...
\begin{tikzpicture}[
  ...
  state/.style={draw, rounded corners=1.5pt, font=\scriptsize\ttfamily,
    inner xsep=4pt, inner ysep=2.5pt, minimum width=22mm},
...
\end{tikzpicture}
\caption{The Seppa optimization loop as served over MCP. Shaded states
(\texttt{hypothesize}, \texttt{implement}) are the only two the agent
owns; the machine executes the rest. Dashed guard edges route compile
failures and physics-gate failures to \texttt{log\_variant}, so
\texttt{benchmark} is reachable only through a green \texttt{verify}.}
\label{fig:fsm}
\end{figure}
