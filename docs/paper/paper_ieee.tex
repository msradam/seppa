...
The claim throughout is machine verification, not autonomous discovery;
this paper reports:
...
multiply, not the stencil this paper optimizes. The GPU is a second,
slower engine that happens to be free. In Vulkan's terms, a dispatch is
one launch of a kernel over the whole grid; one invocation is one
execution of the kernel by one GPU thread; invocations are grouped into
...
number, the model eventually optimizes the measurement rather than the
kernel.
...
the scoring expressed as reachability in a graph rather than a fitness
...
cycles, the scale of starting a thread rather than of arithmetic or
...
run's 883.4, which is the least reproducible quantity in the paper.
Under the earlier campaign the simulation axis is within noise of the
partitioned CPU scheme (711.9 ± 76.1, n=5, against 678.8 ± 10.6, n=25),
so the two-axis claim rests on the later campaign; the decode axis holds
under both (9.6 and 10.3 against 8.4).
...
of that application rather than as part of it. Measurements used its
...
exercise, and a loop that cannot misreport its own verdicts is a
reasonable way to get at it.
