...
failed verification. On the flood-simulation stencil (a kernel in which
each grid cell updates from its neighbors) of an offline flood-guidance
...
The running notes record the matrix-vector result and the GEMM's final
round; the 7.02 baseline is the shipped \texttt{pi/gemm.comp}. None of
it carries the archived logs that back Section VI.
...
C. A run passes when the baseline gates green inside 1,300 to 1,400
...
10.6) and by 14\% on decode (9.6 ± 0.2 vs 8.4 ± 0.1). The slower
...
the simulation margin over the partitioned scheme to within noise (711.9
± 76.1 against 678.8 ± 10.6). The decode margin holds in both campaigns
(9.6 and 10.3 against 8.4), and no measured campaign puts the
