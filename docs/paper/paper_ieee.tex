...
that beats the best measured CPU-only configuration on both axes in the
...
multiply) improved from 7.02 to 13.42 GFLOP/s over two rounds, and a
de-unrolled llama.cpp matrix-vector kernel gained 28\% in end-to-end
text-generation speed. The running notes record the matrix-vector result
and the GEMM's final round; the 7.02 baseline is the shipped
\texttt{pi/gemm.comp}. None of it carries the archived logs that back
Section VI.
...
strip parameter. The model therefore began with the bound named and the
knob exposed, so the session demonstrates the loop's enforcement alone.
The model worked the exposed knob: strip 4 kept at 1,470.6, +8.8\%;
...
load (openssl SHA-256, four threads), the GPU keeps 99\% of its rate and
the load keeps 88\% of its 16 KB-block throughput. Paired with the
...
phases; a solo phase never left 57.6 C. The GPU still kept 99\% and
92\%. Clock governance therefore does not explain decode's 42\% on its
own. The GPU is the steadier side of every pairing except decode, which
