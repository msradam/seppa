...
inference it idles. Two questions decide that idle GPU's worth: whether
it can be made fast enough at a useful workload to be a co-processor,
and whether it can run beside a busy CPU on a shared LPDDR bus. The case
study (Section V) and its machine-checked reproduction (Section VI) take
up the first question; the concurrency measurements (Section VII) take
up the second.
...
as correct, here fixed-tolerance elementwise comparisons, and finds them
systematically optimistic, with seeded-buggy kernels passing the
benchmark oracle while failing a stricter one. SpecBench {[}20{]} states
the general form: when oversight collapses onto an automated suite, the
agent optimizes the suite. Our physics gates are the same kind of
instrument and inherit the same weakness, which is why the limitations
in Section VIII stop short of calling them sufficient.
...
Burr, served over MCP by Theodosia, an adapter that mounts a Burr state
machine as an MCP server, running on the Pi itself.
...
source and host-side parameters) and nothing else. Every measurement and
verdict came from the machine, and no proposed kernel was edited by hand
(transcripts in \texttt{docs/paper/artifacts/claude\_sessions/}).
Concretely: the winning kernel came from the hand-driven sweep and the
machine verified and reproduced it (Section VI); in the one fully
agent-driven session the model tuned only the strip parameter the server
already exposed and never proposed the fused kernel of Section V-B.
...
over more work. Every variant passed all three gates. Speeds are 400
steps at 256x256 on the parameterized \texttt{vkflood2} harness. Its
host loop is faster than the original application's when running
...
(\texttt{\{shader,\ height\_shader,\ strip\}}), the machine found and
...
has headroom. Two other targets ran under the same gates, a GEMM (dense
matrix multiply) improving from 7.02 to 13.42 GFLOP/s over two rounds
and a de-unrolled llama.cpp matrix-vector kernel gaining 28\% in
end-to-end text-generation speed; both are recorded only in the running
notes, without the archived logs that back Section VI.
...
76.3 C; decode alone reached 73.0 C, limited for 3\%. Each window is
long enough for 12 to 28 flood completions (llama-bench generating 64
tokens per repetition, 20 repetitions), and a flood run counts only if
it fits inside the window.

An earlier short-window campaign, preserved in \texttt{conc\_bench2/},
read up to 19\% differently in both directions. We first blamed a
thermal transient, but the logs refute that: the earlier campaign ran
cooler (68.7 C mean against 72.7 C, with less throttling) and still read
lower. Within this window the rate also climbs, from 818.6 steps/s over
the first quarter to 919.1 over the last, so the figure below is a
window mean on a rising curve. Results from the steady-state run of
2026-08-10 are in Table~\ref{tab:conc} (raw logs under
...
likely remains. An operating-system update (kernel 6.12 to 6.18)
separates the July and August campaigns; flood baselines agree across it
(1,346.8 before, 1,348.6 ± 4.1 after). The board itself failed after the
August campaign, so no quantity in this paper can be re-measured.
