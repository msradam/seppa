...
whole grid, and one invocation is one execution of the kernel by one GPU
...
for speed one at a time: instruction scheduling, code motion, loop
...
the subgroup's loads at every row stay on adjacent addresses. The kernel
also consumes each flux value as it is produced; holding all four alive
fails register allocation. The shipped kernel is 1.58x at 256x256: 1.574
to 1.585 across the five scored reproductions, 1.571 in the July
transcript quoted in Section VI, and 1.59x hand-timed in the sweep. It
is 1.41x at 512x512 and 1.18x at 1024x1024, and it holds all gates over
4,000 steps.
...
multiply) improved to 13.42 GFLOP/s over two rounds, and a de-unrolled
llama.cpp matrix-vector kernel gained 28\% in end-to-end text-generation
speed. The running notes record the matrix-vector result and the GEMM's
final round, 12.56 to 13.42 GFLOP/s, whose shaders ship as
\texttt{pi/gemm.comp} and \texttt{pi/gemm-best.comp}. None of it carries
the archived logs that back Section VI.
...
soak brings the chip to working temperature and its repetitions go
unreported. Kernel-alone phases run straight from the cooldown: cool
starts, finished before heat accumulates. The Pi's firmware defends
...
phases landed in different regimes. The concurrent phases ran
soft-limited 70 to 79\% of the time and peaked at 76.3 C; decode alone
reached 73.0 C, limited for 3\%. Each decode phase defines the
...
run counts only if it fits inside the window.
...
reported here sits entirely in the capped regime. Within it the rate
still climbs, from 818.6 steps/s over the first quarter to 919.1 over
the last, so the concurrent rate reported is a window mean on a rising
curve. Results from the steady-state run of 2026-08-10 are in
...
a 29\% tax against the GPU split's 16\%. A third scheme, time-slicing,
would flood about 23\% of the time to match 883 steps/s; it would cap
decode at 8.8 t/s and freeze guidance during bursts. The GPU split beats
the best measured CPU-only option on both axes: by 30\% on simulation
(883.4 ± 47.1 vs 678.8 ± 10.6) and by 14\% on decode (9.6 ± 0.2 vs 8.4 ±
0.1). The slower processor still wins, because the CPU has no spare
cycles to sell. The margin is campaign-dependent, but the direction is
not: in the earlier campaign the decode axis holds and the simulation
axis ties within noise (Section VIII); no measured campaign puts the
