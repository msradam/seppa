...
The gates are necessary conditions, not proof of correctness:
verification covers one storm scenario at one grid size. The keep
decision rests on a single timing sample against a 1\% threshold. Across
Section VI's five runs the baseline spread was about 0.3\%, and the kept
kernel repeated to the timer's millisecond resolution, which at 0.188 s
is 0.53\% per digit. The 3,852 steps/s CPU figure assumed a
cache-resident 256x256 working set and may not hold at larger grids.
...
governance accounts for part of the 16\% decode cost. And the
steady-state campaign is a single run: an earlier campaign at the same
configuration, kept in \texttt{conc\_bench2/}, read 711.9 steps/s
concurrent against this run's 883.4, a gap the logs do not explain.
Under the earlier campaign the simulation margin over the partitioned
scheme narrows to within noise (711.9 ± 76.1 against 678.8 ± 10.6). The
decode margin holds in both campaigns (9.6 and 10.3 against 8.4), and no
