...
parallel. The model proposes changes; a finite-state machine compiles,
verifies against physics-based correctness gates, benchmarks, and
decides keep or revert, and it refuses to benchmark any kernel that
failed verification. The target is the flood-simulation stencil (a
kernel in which each grid cell updates from its neighbors) of an offline
flood-guidance application for the Raspberry Pi 5. On that stencil, a
hand-driven sweep through the same gates produced a kernel 1.58x faster
than the original. The machine re-measured and re-judged the result in
five consecutive runs driven from a second computer, and in every run it
refused to benchmark a kernel that failed its gate. With CPU inference
running concurrently the advantage widens to 2.20x. The deployment
sustains 9.6 tokens/s of language-model decode (token-by-token text
generation) beside 883 simulation steps per second, an operating point
that beats the strongest CPU-only arrangement measured (simulation and
model on separate cores) on both axes in the steady-state campaign.
\end{abstract}
\begin{IEEEkeywords}
...
{[}8{]}. It generates text one token (roughly a word fragment) at a
time, in the phase called decode. AI capability is usually added with
...
reproduction (Section VI) take up the first question; the concurrency
measurements (Section VII) take up the second.
...
by Mesa's v3dv driver {[}9{]}. In this paper, kernel means a GLSL
compute shader.
...
the stencil itself). The GPU is a second, slower engine that happens to
be free. In Vulkan's terms, a dispatch is one launch of a kernel over
the whole grid, and one invocation is one execution of the kernel by one
GPU thread. Invocations are grouped into workgroups, and the hardware
runs invocations in lockstep bundles of 16 called subgroups.
...
strategies {[}16{]}, disabling, one at a time, the optimizations that
spend registers for speed: instruction scheduling, code motion, loop
unrolling, load sorting, pipelining. It then halves the thread count,
...
TritonRL {[}18{]} puts the difficulty in its title, training a model to
write kernels in Triton (a Python-like kernel language) ``without
...
real work did, and inflating the reported speedup to 18x. Containment
took a reward checker, a database of known hacks, and forced stream
...
the physics gates, ran the hand-driven sweep of \mbox{Section V-B}, and
supervised sessions. The language model was Claude Sonnet 5 (Anthropic),
driven through the Claude Code client at high reasoning effort. It
answered each \texttt{hypothesize} prompt by writing
\texttt{implement}'s inputs, the kernel source and the host-side
parameters, and nothing else. Every measurement and verdict came from
the machine, and no proposed kernel was edited by hand (transcript in
...
application's, about 1,345 steps/s against 1,045. Mixing the two
harnesses would inflate every ratio, so every comparison in this paper
is within \texttt{vkflood2}. Table~\ref{tab:sweep} shows
...
near 2 ns per invocation. That is about two clock cycles, the scale of
starting a thread itself. Two hardware details shaped the final kernel.
Strips run vertically. A subgroup's 16 lanes (its 16 invocations) are
horizontal neighbors, so when each lane walks down its own short column,
the subgroup's loads at every row stay on adjacent addresses, the
pattern the memory system serves most efficiently. The kernel also
consumes each flux value as it is produced; holding all four alive fails
...
hardware has headroom. Two other targets ran through the same gated
loop, each behind its own correctness gate. A GEMM (dense matrix
...
loop gained 28\% in end-to-end text-generation speed. The running notes
record the matrix-vector result and the GEMM's final round, 12.56 to
13.42 GFLOP/s, whose shaders ship as \texttt{pi/gemm.comp} and
\texttt{pi/gemm-best.comp}. None of it carries the archived logs that
back Section VI.
...
C. A run passes when the baseline clears its gates inside 1,300 to 1,400
...
requests a fused single-dispatch step. The model therefore began with
the bound named and both parameters exposed, so the session demonstrates
the loop's enforcement alone. The model worked only the strip parameter,
...
alone, and each kernel looping while decode runs. Each decode-bearing
phase gets a cooldown and then a soak of eight decode repetitions; the
soak brings the chip to working temperature, and its repetitions stay
out of the tables. Kernel-alone phases run straight from the cooldown:
cool starts, finished before heat accumulates. The Pi's firmware defends
against heat by lowering clocks, gently past a soft temperature limit
and sharply at a hard throttle, and the logs record which was active.
The soak runs a fixed count instead of waiting on those flags, and the
phases landed in different regimes. Each decode phase defines the
measurement window: llama-bench generates 64 tokens per repetition for
20 repetitions, long enough for 12 to 28 flood completions, and a flood
run counts only if it fits inside the window. Inside those windows the
concurrent phases ran soft-limited 85 to 88\% of the time and carried
the hard-throttle flag for another 10 to 13\%, peaking at 76.3 C; decode
...
clocks. Decode slows from 10.2 to 9.6 tokens/s, so its per-second memory
...
against 10.21, the latter from this run's soak over those same seconds;
notebook 02 derives the alignment. The measured window reported here
sits entirely in the capped regime. Within it the rate still climbs,
from 818.6 steps/s over the first quarter to 919.1 over the last, so the
concurrent rate reported is a window mean on a rising curve. Results
...
\texttt{genload2} in \texttt{docs/paper/artifacts/}. These two pairings
also bound the thermal confound. Both ran at the same 76.3 C, with the
soft limit active for 80\% of the openssl phases and 70\% of the flood
phases; a solo phase never rose above 57.6 C. The GPU still kept 99\%
...
workers synchronize every token, so losing one core stalls all four. The
flood mean is inflated by bursts between decode repetitions, hence its ±
181. Partitioned is the best CPU-only arrangement, and decode there pays
a 29\% tax against the GPU split's 16\%, each tax measured against its
own solo configuration. A third scheme, time-slicing, would flood about
23\% of the time to match 883 steps/s; it would cap decode at 8.8 t/s
and freeze guidance during bursts. The GPU split beats the best measured
CPU-only option on both axes: by 30\% on simulation (883.4 ± 47.1 vs
678.8 ± 10.6) and by 14\% on decode (9.6 ± 0.2 vs 8.4 ± 0.1). The slower
processor still wins, because the CPU has no spare cycles to sell. The
margin is campaign-dependent, but the direction is not: in the earlier
campaign the decode axis holds and the simulation axis ties within noise
(Section VIII); no measured campaign puts the partitioned scheme ahead
on either axis.
...
before, 1,348.6 ± 4.1 after. The board failed after the August campaign,
so none of these quantities can be re-measured.

The gates are necessary conditions: verification covers one storm
scenario at one grid size. What they are built to catch they did catch:
...
resolution; at 0.188 s that granularity is a 0.53\% step. The 3,852
steps/s CPU figure assumed a cache-resident 256x256 working set and may
not hold at larger grids.
...
measurement of the settled operating point exists. The partitioned
counterfactual does replicate: 678.4 ± 63.3, 680.6 ± 22.9, and 678.8 ±
10.6 steps/s across the three campaigns that measured it, with decode at
8.38 and 8.41 in the two that used the shipped hidden-device
configuration. Taken at face value, the earlier campaign still narrows
the GPU split's simulation margin over the partitioned scheme to within
noise: 711.9 ± 76.1 against 678.8 ± 10.6. The decode margin holds in
both campaigns (9.6 and 10.3 against 8.4), and no measured campaign puts
the partitioned scheme ahead on either axis.

The winning kernel came from the hand-driven sweep, and the agent-driven
session worked only the strip parameter of the two the server itself
named, so the loop's ability to discover optimizations on its own is
untested here. The server's \texttt{fork\_at} utility rewinds to an
earlier state; a caller could resample a marginal kernel until a draw
cleared the 1\% threshold, \texttt{evaluate} would not notice, and no
archived session did so. Full GPU offload of the language model is
blocked by an upstream llama.cpp Vulkan defect. Some numbers have no
archived log: the original application's 1,045 steps/s, the sweep's
GFLOP/s column and hand-timed ratios, the 1.41x and 1.18x at larger
grids, and the GEMM and matrix-vector results. These come from the
repository's dated running notes and shipped kernel files.
...
makes a safety decision; it only narrates what the deterministic code
...
\texttt{github.com/msradam/seppa}. That harness, the gates, the
optimized kernel, and every measurement reported here are new work for
this course, built on top of that application. Measurements used its
...
times. In the steady-state campaign that advantage survives deployment:
