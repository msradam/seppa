...
The campaign turned up one configuration surprise first. With any Vulkan device visible, llama.cpp places CPU-resident model weights in the GPU's host-pinned, write-combined memory even at `-ngl 0`, memory that is slow for the CPU reads that dominate decode. Hiding the device with `GGML_VK_VISIBLE_DEVICES=99` took decode from 9.40 ± 0.36 to 11.44 ± 0.20 tokens/s, a 22% gain from an environment variable (A/B from cool starts, five repetitions each). Every number below uses the hidden-device configuration, which the application ships.
