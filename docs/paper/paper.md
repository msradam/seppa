...
Two other targets ran through the same gated loop, each behind its own correctness gate. A GEMM (dense matrix multiply) improved to 13.42 GFLOP/s over two rounds, and a llama.cpp matrix-vector kernel with its unrolled inner loop replaced by a plain loop gained 28% in text-generation speed. The running notes record the matrix-vector result and the GEMM's final round, 12.56 to 13.42 GFLOP/s, and the shaders ship as `pi/gemm.comp` and `pi/gemm-best.comp`.
...
The sample application [7] simulates surface flooding over real terrain [1], finds shelter routes by mobility profile [2], and explains the result in plain language, designed to function in offline scenarios. The model only narrates what the deterministic code computed and does not make a safety decision. The application lives at `github.com/msradam/bonbibi` and was built as an Arm AI Optimization Challenge entry for August 2026; the harness discussed in this paper is located at `github.com/msradam/seppa`. Measurements used the Granite 4.0 1B configuration of the representative application. The model file (a 1.63 B-parameter GGUF, llama.cpp's weight format) appears in earlier llama-bench logs under a granite-3B size label; the application has since moved to a larger model. Build commands are in `pi/flood/README.md`.
