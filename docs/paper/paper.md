...
One session was fully agent-driven (2026-08-02, a three-experiment budget given to the proposer). The server's `characterize` payload names fixed per-invocation cost as the bound and exposes the strip parameter. The model therefore began with the bound named and the knob exposed, so the session demonstrates the loop's enforcement alone. The model worked the exposed knob: strip 4 kept at 1,470.6, +8.8%; strip 8 hit the register cliff and was reverted at 921.7; strip 6 probed it and was reverted at 1,298.7. It never reached fusion, and all of its reported numbers came from the machine.
...
Decode is also the GPU's worst case. Paired with a compute-bound CPU load (openssl SHA-256, four threads), the GPU keeps 99% of its rate and the load keeps 88% of its 16 KB-block throughput. Paired with the memory-bound four-thread CPU flood, the GPU keeps 92% and the load keeps 75%. Both pairings used continuous flood-simulation probes over cool-start 60 s windows, and both sides are logged under `genload2` in `docs/paper/artifacts/`. These two pairings also bound the thermal confound. Both ran at the same 76.3 C, with the soft limit active for 80% of the openssl phases and 70% of the flood phases; a solo phase never left 57.6 C. The GPU still kept 99% and 92%. Clock governance therefore does not explain decode's 42% on its own. The GPU is the steadier side of every pairing except decode, which streams model weights from DRAM every token: interference tracks the CPU load's memory traffic.
