...
This paper shows that a machine can verify, reproduce, and gate the model's results. It does not show that the model can find good optimizations on its own. Given this, this paper reports:
...
4. an interference measurement of the deployed configuration against a gate-verified CPU-only experiment (Section 7). In the steady-state campaign the CPU-GPU split beats the strongest CPU-only arrangement, which gives the simulation and the model separate cores, on both axes.
...
Seppa (Finnish seppä, smith) is a port of AutoKernel [15], an open-source kernel-optimization loop, to a directed-acyclic graph built with Apache Burr. Theodosia, an adapter that mounts a Burr state machine as an MCP server, serves it from the Pi itself.
...
Each decode phase defines the measurement window. llama-bench generates 64 tokens per repetition for 20 repetitions, long enough for 12 to 28 flood completions, and a flood run counts only if it fits inside that window.
...
These two pairings also bound the thermal confound. Both ran at the same 76.3 °C, with the soft limit active for 80% of the openssl phases and 70% of the flood phases, and a solo phase never rose above 57.6 °C. The GPU still kept 99% and 92%, so clock governance does not explain decode's 42% on its own. What separates the three loads is how much memory traffic each one generates: the heavier that traffic, the more the GPU loses, by 1% against openssl, 8% against the CPU flood, and 58% against decode.
...
The loop's ability to discover optimizations on its own is untested here. The winning kernel came from the hand-driven sweep, and the agent-driven session worked only the strip parameter of the two the server itself named. The server's `fork_at` utility rewinds to an earlier state, so a caller could resample a marginal kernel until a draw cleared the 1% threshold, and `evaluate` would not notice. The server can be configured without it, leaving `step` as the only tool, and no archived session used it.
