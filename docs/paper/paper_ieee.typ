...
correctness verification, benchmarking, and the keep-or-revert verdict,
refusing out-of-order transitions so that an unverified kernel can never
reach a benchmark. Recent kernel-generation systems document pervasive
reward hacking, remedied in every published case by machine-owned
checks; Seppa makes that remedy structural. Applied to the
flood-simulation stencil of Bonbibi, an offline flood-guidance
application for the Raspberry Pi 5, the loop produced a fused,
strip-mined V3D kernel 1.59x faster at 256x256, verified by three
physics gates: fp64 NMSE, mass conservation, and basin pooling. The
result survived a false plateau whose cause was structural: every
winning change lay in the host contract, outside the original
shader-only action space. The machine re-derived the result over the
Model Context Protocol, including refusing to benchmark a deliberately
mass-violating kernel. Under concurrent CPU inference the kernel's
...
is modest: 256 maximum invocations per workgroup, 16 KB of shared
memory, a fixed subgroup width of 16, no usable fp16 arithmetic, and no
...
CPU concurrently runs a street router (a RoutingKit Customizable
Contraction Hierarchy \[2\]) and a small language model (Granite 4.0 1B,
CPU-only llama.cpp \[8\]) that narrates the computed route.
Deterministic code owns the safety-critical logic; the model only
phrases it. The GPU flood simulation is what the idle silicon buys, and
its throughput determines how much simulation the system can afford per
routing update.

This paper is about how that GPU code got fast, and about the harness
that made the process trustworthy. Optimizing kernels for V3D is
unforgiving: the driver's shader compiler falls down a
register-allocation fallback ladder on code that any desktop GPU would
accept, workgroups over 256 invocations corrupt silently rather than
failing loudly, and intuitions imported from CUDA-class hardware (unroll
more, prefetch more, tile more) are frequently wrong here, which we know
because we measured them being wrong. An optimizer working on this
target needs two things: a proposal engine that can absorb unfamiliar
constraints quickly, and a verifier that cannot be talked out of the
way.

Seppa supplies both, with an explicit separation of powers. A large
language model owns only the `hypothesize` and `implement` steps of an
optimization loop. A Burr \[5\] finite-state machine owns everything
downstream: `compile`, `verify`, `benchmark`, `evaluate`, `log_variant`.
The machine is served over the Model Context Protocol \[6\], and its
single `step` tool refuses out-of-order calls, so the agent cannot skip
verification or read a benchmark for a kernel that failed correctness.
Correctness is not a convention the model is asked to follow; it is a
transition the machine will not take.
...
definition. Yet the record of the systems built on it is largely a
record of reward hacking. Kevin \[11\], trained with multi-turn RL,
learned to wrap and inherit the PyTorch reference and required
externally enforced guardrails. CUDA-L1 \[12\] reports that 82 of its
250 RL-generated kernels (32.8%) exploited stream-timing loopholes to
fake 18x speedups, and needed layered anti-hacking machinery (reward
checkers, a hacking-case database, forced stream synchronization) to
contain it. TritonRL \[13\] finds delegation-based cheating across
systems and names the discrepancy "a critical gap in reward fidelity."
...
fixed-shape, small-sample allclose checks that certify buggy kernels as
correct, and argues for fuzzing against an fp64 CPU reference with
per-operator tolerances. Seppa's gates are of the stronger kind: an fp64
CPU reference (NMSE) plus domain invariants, mass conservation against
injected rainfall and physically correct pooling, that a numerically
plausible but wrong kernel cannot satisfy.

Closer to hardware, LLMs have produced tapeout-grade Verilog
conversationally (Chip-Chat \[16\]) and as fine-tuned generators
(VeriGen \[17\]), with humans and testbenches closing the verification
loop; Seppa moves that loop into a machine-owned FSM. FunSearch \[4\]
and AlphaEvolve \[9\] pair LLM proposal with automated evaluators inside
evolutionary searches and credit the evaluator, not the model, with the
...
parallelism), Customizable Contraction Hierarchies \[2\] for reroutable
...
shared, subgroup 16). `implement` accepts the agent's proposal.
Everything else is deterministic machinery with no model in the loop:
compilation via glslangValidator, verification and benchmarking via the
target harness, and an `evaluate` step that keeps a variant only if it
beats the best by more than 1%, persisting the winner to disk. Three
consecutive non-improvements, or the experiment budget, ends the run.
The machine's ledger (`variant_log`) records every experiment, kept or
reverted, with its gate results.

For the flood target, `verify` runs the physics gate of the `vkflood2`
harness: 400 steps at 256x256 must satisfy (a) NMSE below 1e-3 against a
double-precision CPU reference of the same WCA2D update, (b) total water
equal to injected rainfall (mass conservation), and (c) maximum depth
pooling inside the terrain basin. The benchmark metric is simulation
...
1,045 steps/s, 1.51 GFLOP/s), with both shaders compiling cleanly.
Notably, the shaders are too small to stress the v3dv register
...
three physics gates; speeds are for 400 steps at 256x256 on the
parameterized `vkflood2` harness. Its host loop is itself faster than
the original harness for identical kernels (about 1,345 steps/s, 1.94
GFLOP/s, against the 1,045 above), so every comparison in this paper is
within `vkflood2`.
...
#figure(
  align(center)[#table(
    ...
    table.header([Variant], [Hypothesis tested], [Result],),
    table.hline(),
    [Packed vec2 state (water, surface) halving flux-pass
    loads], [Memory-op count is the bound], [1.93 GFLOP/s, no change:
    falsified],
    [Fused single-dispatch step (flux buffer eliminated)], [Traffic and
    barriers are the bound], [2.04 GFLOP/s, +5%: mostly falsified],
    [2 cells per invocation (strip-mined dispatch)], [Fixed
    per-invocation cost is the bound], [2.40 GFLOP/s, +23%: supported],
    [4 cells per invocation], [More strip is better], [2.35 GFLOP/s:
    falsified, register pressure],
    [Fused + 2 cells per invocation (`fused2s.comp`)], [The wins
    stack], [3.08 GFLOP/s, +59%],
  )]
  , kind: table
  )

The stencil is bound by fixed per-invocation cost: each invocation does
so little arithmetic that launch overhead dominates, and amortizing it
over two cells is worth more than eliminating half the memory system's
work. Two further V3D specifics shaped the winner. Vertical strips keep
a subgroup's 16 lanes on adjacent x addresses, and lane contiguity
matters more on this GPU than per-thread load count. And the kernel had
to be written for the register allocator: holding all four flux vectors
of the cell pair live fails v3dv register allocation and falls down the
compiler's fallback ladder, so the shipped kernel consumes each flux as
it is produced and keeps only four scalars.

The shipped kernel is 1.59x at 256x256 (0.187 s vs 0.297 s), 1.41x at
512x512, 1.18x at 1024x1024, and holds all gates over a 4,000-step run
(NMSE 1.3e-9, mass conserved, pools in basin). Reproduction:
...
weights in the GPU's host-pinned, write-combined memory even at
`-ngl 0`, which is fast for GPU transfers that never happen and slow for
...
portable rule: a CPU-only llama.cpp process on a board with an
integrated GPU should hide that GPU.

`pi/flood/concurrency_bench.sh` measures six conditions on the Pi: each
flood kernel alone (4,000-step runs), CPU decode alone (llama-bench
tg64, 5 repetitions, 4 threads), the router alone, and each flood kernel
looping continuously while the same decode benchmark runs, plus the
router under flood load. Every phase starts from below 55 C, and a 1 Hz
sampler records temperature and the throttle register throughout;
`pi/flood/analyze_conc_bench.py` reduces the raw logs and reports
per-phase thermal validity. Concurrent flood runs count only if they
start and finish inside the decode window. Results from the run of
2026-07-12 (raw logs in `docs/paper/artifacts/conc_bench2/`\; the
earlier GPU-visible run is preserved in `conc_bench/`):

#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([Condition], [GPU flood (steps/s)], [CPU decode (t/s)],),
    table.hline(),
...
The router is not in the table because it does not move: `route.py`
completes in 0.02 s alone and 0.02 s under full GPU flood load. The n=2
on the original-kernel concurrent row is a consequence of its own
slowness: only two 4,000-step runs fit inside the decode window.

Three results. First, the co-processing thesis holds with a measured
price. Adding the optimized flood loop to a board already saturated with
CPU decode costs the CPU about 10% of its decode rate and buys a
continuous 712 steps/s flood simulation that a CPU-only deployment
simply does not have. Second, interference is strongly asymmetric, and
...
sampling; raw logs in `docs/paper/artifacts/cpu_flood_bench2/`.
...
#figure(
  align(center)[#table(
    ...
    table.hline(),
    [CPU flood alone, 1 / 2 / 4 threads], [992.0 / 1,977.9 /
    3,803.2], [],
    [Decode alone, 3 threads (pinned)], [], [11.8 ± 0.0],
    [Partitioned: flood 1t + decode 3t], [680.6 ± 22.9 (n=6)], [8.4 ±
    0.0],
    [Oversubscribed: flood 4t + decode 4t], [703.4 ± 338.7 (n=21)], [2.4
    ± 0.3],
    [GPU concurrent, optimized (from above)], [711.9 ± 76.1], [10.3 ±
    0.2],
  )]
  , kind: table
  )
...
four idle A76 cores run this stencil at 3,803 steps/s (5.4 GFLOP/s,
...
+ Optimize for the driver's register allocator first. v3dv compiles
  through a fallback ladder (disabling scheduling, then loop unrolling,
  then thread count) and the performance cliff is in the ladder, not in
  your arithmetic. Source-level de-unrolling rescued llama.cpp's
  matrix-vector kernel (+28% end-to-end decode); accumulator reduction
  took a GEMM kernel from 7.02 to 13.42 GFLOP/s in the same FSM under an
  NMSE gate.
+ Fixed per-invocation cost dominates small kernels. Strip-mining beat
  every memory-system optimization on the flood stencil, and 2 cells per
  invocation was the sweet spot before register pressure took the gain
  back.
+ Lane contiguity beats per-thread operation count. A transposed-B GEMM
  variant with fewer per-thread loads was verified correct and 36%
  slower.

The converse also holds: the de-unroll playbook did nothing for the
flood stencil because its shaders never stressed the allocator. The
...
that serves this paper as workload and demonstration vehicle. The
contributions reported here, made beyond that entry, are: the
correctness-gated FSM harness and its MCP serving; the verified
kernel-optimization ledger including the falsified hypotheses and the
action-space result; the machine-checked reproduction; the concurrency
envelope and its CPU-only counterfactual; and this paper's analysis. The
application's interface, mobility-threshold research, and deployment
materials are documented in the Bonbibi repository and its competition
submission. The dual use of one project for the course deliverable and
...
\[2\] J. Dibbelt, B. Strasser, and D. Wagner. Customizable Contraction
Hierarchies. ACM Journal of Experimental Algorithmics 21, Article 1.5,
\2016.

\[3\] A. Ouyang, S. Guo, et al.~KernelBench: Can LLMs Write Efficient
GPU Kernels? arXiv:2502.10517, 2025.

\[4\] B. Romera-Paredes et al.~Mathematical discoveries from program
search with large language models. Nature 625:468-475, 2024.

\[5\] Burr: build applications that make decisions. DAGWorks / Apache.
...
\[9\] AlphaEvolve: a coding agent for scientific and algorithmic
discovery. Google DeepMind, 2025.

\[10\] V3D driver documentation, The Mesa 3D Graphics Library.
https:/\/docs.mesa3d.org/drivers/v3d.html

\[11\] C. Baronio, P. Marsella, B. Pan, S. Alberti, S. Ehrlich et
al.~Kevin: Multi-Turn RL for Generating CUDA Kernels. arXiv:2507.11948,
\2025.

\[12\] X. Li et al.~CUDA-L1: Improving CUDA Optimization via Contrastive
Reinforcement Learning. arXiv:2507.14111; ICLR 2026.

\[13\] TritonRL: Training LLMs to Think and Code Triton Without
Cheating. arXiv:2510.17891, 2025.

\[14\] B. Zhao, D. Srikanth, Y. Wu, and Z. Jiang. SpecBench: Measuring
Reward Hacking in Long-Horizon Coding Agents. arXiv:2605.21384, 2026.

\[15\] D. Sarkar. The Correctness Illusion in LLM-Generated GPU Kernels.
arXiv:2606.20128, 2026.

\[16\] J. Blocklove, S. Garg, R. Karri, and H. Pearce. Chip-Chat:
