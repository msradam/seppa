...
language-model inference and street routing. Adding the GPU flood loop
to a board saturated with 4-thread LLM decode costs the CPU about 15% of
its decode rate (9.5 to 8.1 tokens/s) and buys a continuous 1,100
...
2026-07-11 (raw logs in `docs/paper/artifacts/conc_bench/`):
...
#figure(
  align(center)[#table(
    ...
    table.hline(),
    [Optimized kernel alone], [2,126.9 ± 0.7 (n=3)], [],
    [Original kernel alone], [1,348.2 ± 1.6 (n=3)], [],
    [Decode alone], [], [9.5 ± 0.2],
    [Concurrent, optimized kernel], [1,103.1 ± 145.2 (n=8)], [8.1 ±
    0.4],
    [Concurrent, original kernel], [599.5 ± 81.2 (n=6)], [7.7 ± 0.6],
  )]
  , kind: table
  )

The router is not in the table because it does not move: `route.py`
completes in 0.02 s alone and 0.02 s under full GPU flood load.
...
CPU decode costs the CPU about 15% of its decode rate and buys a
continuous 1,100 steps/s flood simulation that a CPU-only deployment
simply does not have. Second, interference is strongly asymmetric: the
CPU keeps 85% of its solo throughput, but the GPU keeps only 52%. The
two halves share one LPDDR memory system, and the GPU is the victim of
that sharing. "Idle silicon" is real but it is not free silicon; it is
roughly half a GPU when the CPU is busy. Third, the kernel optimization
matters more under contention, not less: the optimized kernel's
advantage over the original grows from 1.58x alone to 1.84x concurrent,
and it costs the CPU less (decode retains 8.1 t/s against 7.7 t/s under
the original kernel). Both effects are consistent with the fused
kernel's elimination of the intermediate flux-buffer traffic: fewer
bytes through the shared memory system per simulation step means less to
lose when bandwidth is contended and less pressure exerted on the CPU's
...
throttled during the decode-alone phase, peak 74.7 C; 29 of 43 and 31 of
45 samples during the concurrent phases, peak 76.3 C). Cool-throughout
sustained measurements are unattainable on this cooling, so the numbers
above are steady-state, thermally governed figures rather than
cold-silicon peaks. For the deployment question this paper asks, that is
