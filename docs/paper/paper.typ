...
<optimizing-for-idle-silicon-correctness-gated-llm-driven-kernel-optimization-on-the-raspberry-pi-5s-integrated-gpu>
Adam Munawar Rahman New York University. Code: github.com/msradam/seppa,
github.com/msradam/bonbibi

== Abstract
<abstract>
Single-board computers ship with integrated GPUs that most edge AI
workloads leave idle: inference runs on the CPU and the GPU does
nothing. We present a case study in putting that silicon to work. Seppa
is a kernel-optimization harness in which a large language model
proposes GPU kernel and host-contract changes, and a finite-state
...
second) with physics verified by three independent gates: numerical
agreement with a double-precision CPU reference, mass conservation
against injected rainfall, and correct pooling in a terrain basin. The
optimization survived an initial false plateau: a first search over
shader bodies alone found nothing, because every winning change lived in
the host contract (buffer packing, kernel fusion, dispatch geometry).
Widening the machine's action space to include that contract recovered
the full speedup, and the machine then re-derived the result end to end
over the Model Context Protocol, including refusing to benchmark a
deliberately mass-violating kernel. Finally, we measure the point of the
exercise: the optimized flood kernel running concurrently with CPU-side
language-model inference and street routing. The measurement itself
surfaced an optimization: with a Vulkan device visible, llama.cpp places
CPU weights in GPU host-pinned write-combined memory even at zero
offload, costing 22% of decode throughput (9.40 to 11.44 tokens/s when
the device is hidden); all figures below use the corrected
configuration. Adding the GPU flood loop to a board saturated with
4-thread LLM decode then costs the CPU about 10% of its decode rate
(11.5 to 10.3 tokens/s) and buys a continuous 712 steps/s simulation;
the GPU, sharing one LPDDR memory system with a now bandwidth-hungrier
CPU, retains 33% of its solo throughput. The kernel optimization matters
more under contention, not less: its advantage over the original kernel
grows from 1.58x alone to 2.09x concurrent. A gate-verified CPU-only
counterfactual (the same update in OpenMP) sharpens the claim: four idle
cores outrun the GPU on this stencil, but with decode running, the best
CPU-only scheme reaches 8.4 tokens/s with 681 steps/s while the
GPU-concurrent split delivers 10.3 and 712, dominating partitioned,
oversubscribed, and time-sliced alternatives on both axes. All numbers
in this paper carry their exact reproduction commands.
...
== 2. Related work
<related-work>
LLM-driven kernel generation is an active area. KernelBench \[3\]
evaluates models writing CUDA kernels for 250 PyTorch workloads and
finds frontier models match the baseline in under 20% of tasks out of
the box, improving with execution feedback. FunSearch \[4\] and
AlphaEvolve \[9\] pair LLM proposal with automated evaluators inside an
evolutionary loop, and credit the evaluator, not the model, with the
reliability of the results. Seppa shares that architectural conviction
and differs in three ways: the target is a hostile, sparsely documented
embedded GPU rather than a well-modeled datacenter part; the evaluator
is a stateful FSM whose transition structure (not just its scoring)
enforces the discipline, served to the agent as a remote tool it cannot
bypass; and the action space includes the host contract, which Section
4.3 shows is where the wins were.
