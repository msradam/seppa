# Seppa: AutoKernel Ported to the V3D GPU
...
> measured is simpler than the MNN/llama.cpp-integrated plan below: the
> optimization target is a standalone GEMM shader (`best_gemm.comp`,
> baseline `pi/gemm.comp`), evaluated by a small dedicated harness
> (`pi/vkgemm_nmse.cpp`), not an op inside MNN or llama.cpp's `mul_mat`. The
...
AutoKernel is not an agent framework: it is a set of stateless Python scripts plus a 909-line `program.md` prompt, and the "agent" is a human-launched LLM agent that edits `kernel.py`, runs `bench.py`, and does keep/revert with plain `git`. Seppa keeps AutoKernel's *shape* (profile, extract, edit, compile, verify, benchmark, keep/revert, log, one artifact at a time) but replaces its entire torch/CUDA execution substrate, because every correctness check in `bench.py` is `torch.allclose(kernel_fn(**inputs), pytorch_reference)` on a CUDA tensor and none of that exists on V3D. The 10-state Burr FSM lives at `~/v3d-pi-ai/burr_fsm/fsm.py`, runs on the Pi via `subprocess`, and encodes the prior audit's correctness lesson in its `verify`/`evaluate` gates. Two pieces sit on top of it: (a) AutoKernel's file-loop discipline and playbook ported into that FSM's `hypothesize`/`implement` actions, and (b) the FSM wrapped with Theodosia so a laptop agent drives it over MCP.
...
| `orchestrate.py` | Reusable **almost verbatim**, pure Python bookkeeping (JSON state, Amdahl `1/(1-Σfᵢ(1-1/sᵢ))`, move-on criteria, TSV append) with **zero torch and zero git calls**. Retarget its metric from `tflops` to `tok/s` (or `bw_util`) and its move-on thresholds to V3D numbers. |
...
- **Compile:** `glslangValidator -V -Os kernel.comp -o kernel.spv` (V3D toolchain uses `glslangValidator`, confirmed in the prior build logs; `glslc` is the equivalent and also fine). The trap is downstream: MNN and llama.cpp link **pre-compiled SPIR-V byte arrays** (`AllShader.cpp`, `mul_mm.comp.cpp`) and `cmake --build` has **no dependency rule from `.comp` to those arrays**. This is the entire root cause of the prior audit failure. So the COMPILE state runs the *full* offline chain (`glslangValidator` → `makeshader.py`/`spirv-as` repack → relink) and fails loudly if the repack is skipped. A stale-bytecode guard (hash the `.spv`, assert it changed) belongs here.
...
## 4. Correctness harness: the five stages, and the audit lesson
...
| 5. Edge cases | non-power-of-2 sizes | V3D's real edge is `local_size` not dividing the work domain (the exact prior failure mode: partial output, undefined tail). Add an assertion that **all** output elements are written (compare full output image, not a sampled tile) |

**The prior audit's finding IS the harness design constraint.** MNN's autotuner crowned a shader candidate that did `1/K` of the work and ran ~K× faster, because *the autotuner times candidates with no reference-output check*. AutoKernel's whole architecture ("correctness gate before you ever look at the number, revert instantly on FAIL") is the antidote, and it is exactly what the prior investigation lacked. So the V3D VERIFY state must:
...
| **4. Row/channel packing** | output channels per invocation | register-bound: threads=4 needs ≤32 regs, ~64-reg envelope; **this is the tier the prior bug lived in**, packing must write *all* channels |
...
Anti-patterns section (analog to AutoKernel's): channel packing without full-output verification (the prior audit), assuming fp16 ALU speedup (there is none), random-gather access patterns (36× bandwidth collapse: 4.0 → 0.11 GB/s), workgroups > 256, shared mem > 16 KB, forgetting the host-side `wg_denoms` co-change.
...
| *(implicit in torch/Triton JIT)* | `COMPILE`, **new state, V3D-specific**: the offline SPIR-V repack that the prior audit proved you cannot skip |
...
- `verify → log_variant` on `not verify_ok` (correctness fail **bypasses** benchmark/evaluate, so a wrong kernel never gets timed). Keep; this is the audit-derived guard.
...
- **What the agent sees:** every `step` response returns `valid_next_actions` and, on refusal, `next_action_schemas`. The agent also reads `theodosia://graph` (topology + conditions), `theodosia://state`, `theodosia://next`, `theodosia://history`, `theodosia://trace`. A guard failure is a **structured refusal payload** (`{"error":"invalid_transition","valid_next_actions":[…]}`), not a crash, so the agent recovers by picking a legal action.
- **Where it runs (decisive design choice):** Theodosia runs on aarch64 / Python 3.13 (all runtime deps pure-Python or have prebuilt aarch64 wheels; `psutil` is dev-only). The FSM runs on the Pi via local `subprocess`. So **the Theodosia MCP server runs on the Pi** (`theodosia serve fsm:build_app --transport streamable-http --host 0.0.0.0 --port 8000`) and the laptop agent's MCP client points at `http://<pi>:8000/mcp`. The FSM calls local binaries with zero SSH inside the FSM code. This is the simplest correct topology: no SSH marshaling in the hot loop, the Pi owns its own tools.
- **SSH's remaining role:** provisioning and `bench.sh`-from-laptop fallback only. In the Theodosia-on-Pi topology, the FSM's `benchmark`/`compile` actions run `subprocess.run([...])` locally (already true in `benchmark.py`, `probe.py`); SSH is used by humans for setup (`scp`/`ssh root@<pi-ip>`, the Pi from the investigation notes).
...
- **Theodosia ledger + Burr tracker** = the decision layer. Every `step` (including refusals, such as a rejected `benchmark` on an unverified candidate) is recorded immutably. This is what a paper needs: proof that no timed result ever bypassed the correctness gate. The ledger records *transitions*; git records *diffs*.
...
**Optimize `convolution1x1.comp` (MNN's shader, the exact culprit from the prior audit) for V3D's 256-invocation limit, through the Theodosia-driven FSM, and prove the loop rejects the partial-compute candidate.**
...
4. One HYPOTHESIZE→IMPLEMENT→COMPILE→VERIFY cycle where the agent tries a channel-packing variant (`gws.y = UP_DIV(ocDiv4, K)`), and **VERIFY rejects it** because `verify_complete` fails (not all channels written), reproducing the prior audit's catch, this time *inside the gate*.
5. A second cycle with a legitimate `local_size` change (e.g. `(1,240,1)`) that VERIFIES and is kept or reverted on honest t/s.

Success = the ledger shows a benchmarked kept variant **and** a refused partial-compute variant. That single trace is the paper's core claim: the loop makes that failure structurally impossible.
